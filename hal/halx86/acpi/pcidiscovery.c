    ...
    _In_ UCHAR EcamStartBus)
{
...

    {
        ...
        if (PciData->Command & PCI_ENABLE_BUS_MASTER)
            p += sprintf(p, " bus master,");
        if (PciData->Status & PCI_STATUS_66MHZ_CAPABLE)
            p += sprintf(p, " 66MHz,");
        if ((PciData->Status & PCI_STATUS_DEVSEL) == 0x000)
            p += sprintf(p, " fast devsel,");
        else if ((PciData->Status & PCI_STATUS_DEVSEL) == 0x200)
            p += sprintf(p, " medium devsel,");
        else if ((PciData->Status & PCI_STATUS_DEVSEL) == 0x400)
            p += sprintf(p, " slow devsel,");
        else if ((PciData->Status & PCI_STATUS_DEVSEL) == 0x600)
            p += sprintf(p, " unknown devsel,");
        p += sprintf(p, " latency %d", PciData->LatencyTimer);
        if (PciData->u.type0.InterruptPin != 0 &&
