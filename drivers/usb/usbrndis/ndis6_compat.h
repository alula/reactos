...
/*
 * NOTE: NDIS_MINIPORT_MAJOR_VERSION and NDIS_MINIPORT_MINOR_VERSION
 * are defined by ndis.h based on NDIS620_MINIPORT or similar macros.
 * If ndis.h is included before this header (as it should be), these
 * macros will already be set correctly. We only define fallbacks here
 * for cases where ndis.h was not included first.
 */
