...
#ifdef __REACTOS__
static UINT32 dinput_hid_path_hash( const WCHAR *path )
{
...
#else
    if (!HidD_GetProductString( device_file, instance->tszInstanceName, MAX_PATH * sizeof(WCHAR) ))
        swprintf( instance->tszInstanceName, MAX_PATH, L"HID Joystick %04x:%04x", attrs->VendorID, attrs->ProductID );
    if (!HidD_GetProductString( device_file, instance->tszProductName, MAX_PATH * sizeof(WCHAR) ))
        lstrcpynW( instance->tszProductName, instance->tszInstanceName, MAX_PATH );
#endif

#ifdef __REACTOS__
    handle = dinput_hid_path_hash( path );
#else
