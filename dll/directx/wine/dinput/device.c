                                                  ...
                                                  const WCHAR *username, DWORD flags )
{
...
#ifdef __REACTOS__
    BOOL empty_action_map_changed = FALSE;
#endif
    int i, index;
    HRESULT hr;
...
#ifdef __REACTOS__
    if (!data_format.dwNumObjs)
    {
        empty_action_map_changed = impl->has_empty_action_map &&
                                   (impl->empty_action_data_size != data_format.dwDataSize ||
                                    impl->empty_action_count != format->dwNumActions);
        impl->has_empty_action_map = TRUE;
        impl->empty_action_data_size = data_format.dwDataSize;
        impl->empty_action_count = format->dwNumActions;
    }
    else
    {
        impl->has_empty_action_map = FALSE;
    }
#endif
    ...
    if (!data_format.dwNumObjs)
    {
        if (empty_action_map_changed) return DI_OK;
        return DI_NOEFFECT;
    }
    return hr;
#else
